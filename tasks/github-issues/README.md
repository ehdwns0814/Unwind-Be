...
- [ ] **환경변수 설정이 완료되었는가? (`.env` 파일 생성 및 설정)**
...
## 🔒 보안 관리 방침 (Security Policy)

개발 및 배포 과정에서의 보안 강화를 위해 다음 원칙을 준수합니다:

1. **민감 정보 하드코딩 금지**: DB 비밀번호, JWT Secret, API Key 등은 절대 코드나 설정 파일(`application.yml`)에 직접 작성하지 않습니다.
2. **환경변수 활용**: 모든 민감 정보는 `.env` 파일 또는 시스템 환경변수를 통해 주입받습니다.
3. **로컬 설정 보호**: `.env` 및 `application-local.yml` 파일은 `.gitignore`에 등록되어 외부로 노출되지 않도록 관리합니다.
4. **템플릿 제공**: 신규 환경 설정을 위해 `application-local.yml.template`을 참조하여 개인별 로컬 환경을 구축합니다.

---

**최종 수정:** 2026-01-17 (보안 방침 업데이트)  
